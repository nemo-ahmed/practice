function sss(text: string, markers: string[]) {
...
function runTest(text: string, markers: string[], expected: string) {
...
export function run() {
  tests.forEach(([text, markers, expected]) =>
    runTest(text as string, markers as unknown as string[], expected as string),
  );
}
